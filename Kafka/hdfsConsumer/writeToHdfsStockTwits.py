...
def flush_to_hdfs(output_dir, topic):
    ...
    print("/usr/bin/hdfs dfs -put -f %s %s 2> /dev/null" %
          (tempfile_path, hadoop_path))
    os.system("/usr/bin/hdfs dfs -put -f %s %s 2> /dev/null" %
              (tempfile_path, hadoop_path))
    os.remove(tempfile_path)
    batch_counter += 1
    tempfile_path = "/tmp/kafka_%s_%s_%s_%s.dat" % (
        topic, group, timestamp, batch_counter)
...
def consume_topic(topic, group, output_dir, frequency):
    ...
    kafka_consumer = SimpleConsumer(
        kafka, group, topic, max_buffer_size=1310720000)

    # open file for writing
    tempfile_path = "/tmp/kafka_stockTwits_%s_%s_%s_%s.dat" % (
        topic, group, timestamp, batch_counter)
    tempfile = open(tempfile_path, "w")
    ...
    while True:
        ...
        # OffsetAndMessage(offset=43, message=Message(magic=0, attributes=0, key=None, value='some message'))
